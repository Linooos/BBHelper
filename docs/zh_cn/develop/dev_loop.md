...
## 5.11 OCR 读不出**纯数字编号**（1-1 / 2-1）⚠️

2026-09-21 用户报「体力消耗认不出 1-1 / 1-2 的关卡名」。实测结论：

**不是 ROI 的问题，是 OCR 把短横线丢了。**

```text
同一屏、全屏 ROI：
    「1-2」→  text="1-2"   score 0.9878   ✓ 读得出来
    「1-1」→  text="111"   score 0.4743   ✗ 短横线没了
```

用 `expected: ["1"]` 放宽才把真实读数逼出来（`box=[332,243,53,41] text="111"`）——
**排查这类「怎么都对不上」的识别时，把 expected 放宽让它把读到的文本吐出来**，
比盯着 ROI 猜快得多。

两个相同的「1」更容易被模型合并，所以 `1-1` 比 `1-2` 更容易坏。
分数只有 0.47（默认 threshold 0.3 还是会过），属于「检出了但读错了」。

### 两条出路

| 做法 | 说明 |
| --- | --- |
| 填 **OCR 实际读到的文本** | `expected` 是**子串匹配**，填 `111` 能命中同一个框 ✓ 实测过 |
| 切到**按坐标** | 走 `DirectHit`，完全不经过 OCR —— 最稳，推荐 |

### 和「出击」那个坑是同一类

§5 里记的「出击」也是装饰字体读不出（斜排被拆成两字）。
**规律：游戏里为了好看而特殊排版/配色的短文本，OCR 都不可靠。**
凡是这类文本，优先考虑：
1. 换一句**普通字体**的判据（如 §5.10 用「选择助战好友」代替返回箭头）
